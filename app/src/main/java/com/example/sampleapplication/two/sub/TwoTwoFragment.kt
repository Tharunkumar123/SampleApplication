...
import com.example.sampleapplication.two.sub.TwoThreeFragment


class TwoTwoFragment : Fragment() {
    private lateinit var txtSubFragment: TextView
    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        val view = inflater.inflate(R.layout.fragment_two_two, container, false)
        ...
        txtSubFragment.setOnClickListener {
            if(this.parentFragment is TwoActivityCommunicator){
                (this.parentFragment as TwoActivityCommunicator).loadFragment(TwoThreeFragment())
            }
        }
        return view
    }
