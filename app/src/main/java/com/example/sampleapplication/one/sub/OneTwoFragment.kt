...
class OneTwoFragment : Fragment() {
    private lateinit var txtSubFragment: TextView
    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        val view = inflater.inflate(R.layout.fragment_one_two, container, false)
        ...
        txtSubFragment.setOnClickListener {
            if(this.parentFragment is OneActivityCommunicator){
                (this.parentFragment as OneActivityCommunicator).loadFragment(OneThreeFragment())
            }
        }
        return view
    }
